...
function TaskDetail({ taskid, updateTasks }) {
  ...
  const deleteTodo = (todo) => {
    fetch(`http://localhost:5000/todos/byid/${todo._id}`, {
      ...
      .then((res) => res.json())
      .then(() => {
        // Doesn't rerender the DOM in cypress if in separate .then callbacks
        updateTask()
        updateTasks()
      })
      .catch(function (error) {
  ...
  return task == null ? (
    ...
    <div>
      ...
      <ul className="todo-list">
        {todos.map((todo, i) => (
          <li key={todo._id} className="todo-item" data-test={`todo-item-${i}`}>
            <span
        ...
        <li key="newtodo">
          <form onSubmit={addTodo} className="inline-form">
            <input
              ...
              data-test="todo-add-input"
            ></input>
            <input
              type="submit"
              value="Add"
              data-test="todo-add-submit"
            ></input>
          </form>
        </li>
      </ul>
